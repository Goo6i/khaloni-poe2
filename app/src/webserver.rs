...
/// Never fails hard: a fetch error yields an empty index so the panel still
/// runs (the reference tab just shows nothing until the next successful fetch).
...
/// caches it. Empty string on failure so the panel still runs.
fn cached(cache_dir: &std::path::Path, name: &str, fetch: impl FnOnce() -> Result<String, String>) -> String {
